...
for i = 1:numel(suspiciousIdx)
    ...
    if size(coords,1) < 10
        preSplitMasks{end+1} = buildMaskFromIdx(pix, CC.ImageSize);%%%FIX: keep it instead of dropping it
        continue;
    end
    ...
    accepted = 0;%%%FIX: track whether any half was kept
    for c = 1:2
        ...
        elongation_new = elongation_new(isfinite(elongation_new));%%%FIX: single-voxel comps give Inf/NaN
        if nnz(newMask) > 0 && (isempty(elongation_new) || all(elongation_new<8))%%%FIX: && and all(), was & on a vector
            preSplitMasks{end+1} = newMask;
            accepted = accepted + 1;
        end
    end
    if accepted == 0%%%FIX: no half kept -> put the original back, do not lose the voxels
        preSplitMasks{end+1} = buildMaskFromIdx(pix, CC.ImageSize);
    end
end
...
%%%FIX: after the while-loop above, clusterMasks is no longer sorted by size,
%%%FIX: so "the first k" were not "the k largest". Re-sort before selecting.
if numel(clusterMasks) < k
    error('Only %d clusters available after splitting, need %d.', numel(clusterMasks), k);
end
sz_fix = zeros(1, numel(clusterMasks));
for i = 1:numel(clusterMasks)
    sz_fix(i) = nnz(clusterMasks{i});
end
[~, ord_fix] = sort(sz_fix, 'descend');
clusterMasks = clusterMasks(ord_fix);
