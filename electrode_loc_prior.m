...
if  isequal(sort(elec_config), sort({'F4','AF4','F2','FC4','F6'}))
    z_ind(1)=find(squeeze(help(round(size(help,1)/2),round(size(help,2)/2),:)),1,'last');%%z-dir
    z_ind(2)=find(squeeze(help(round(size(help,1)/2),round(size(help,2)/2)+floor(7/vox(1)),:)),1,'last');
    z_ind(3)=find(squeeze(help(round(size(help,1)/2),round(size(help,2)/2)-floor(7/vox(1)),:)),1,'last');
    z_ind_final=max(z_ind)-floor(63/vox(1));
    yb_ind(1)=find(squeeze(help(round(size(help,1)/2),:,z_ind_final)),1);%%y-dir
    yb_ind_final=(yb_ind)+floor(44/vox(3));
    yf_ind(1)=find(squeeze(help(round(size(help,1)/2),:,z_ind_final)),1,'last');
    yf_ind_final=(yf_ind);
    ...
    %%%this code is for the electrode configuration {'F4','AF4','F2','FC4','F6'}, it removes voxels from the right side of the head
...
elseif isequal(sort(elec_config),sort({'F3','AF3','F1','FC3','F5'}))
z_ind(1)=find(squeeze(help(round(size(help,1)/2),round(size(help,2)/2),:)),1,'last');%%z-dir
    z_ind(2)=find(squeeze(help(round(size(help,1)/2),round(size(help,2)/2)+floor(7/vox(1)),:)),1,'last');
    z_ind(3)=find(squeeze(help(round(size(help,1)/2),round(size(help,2)/2)-floor(7/vox(1)),:)),1,'last');
    z_ind_final=max(z_ind)-floor(63/vox(1));
    ...
    xr_ind(1)=find(squeeze(help(:,yf_ind_final,z_ind_final)),1);%for left side remove 'last'
    xr_ind_final=(xr_ind)-floor(32/vox(2));%%+x means move to right
    mask_elec(1:xr_ind_final+floor(48/vox(2)),:,:)=zeros((xr_ind_final+floor(48/vox(2))),size(mask_elec,2),size(mask_elec,3));
    h_help.img=mask_elec;
    save_untouch_nii(h_help,[datapath,'/h2.nii']);%%%this saves a nifty of the location prior
